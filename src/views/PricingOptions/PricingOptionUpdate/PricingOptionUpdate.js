...
import './PricingOptionUpdate.scss'
...
const PricingOptionUpdate = ({ match }) => {
    ...
    const getItem = async (id) => {
      ...
      try {
        const responsee = await fetch(
          `${global.apiUrl}api/offerAndPriceTypes?is_price_type=1&type_id=${match.params.offer}`,
          {
            method: "GET",
            headers: {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const responsee = await fetch(
        `${global.apiUrl}api/offerTypes/${match.params.id}?_method=put`,
        {
          method: "POST",
          headers: {
            Authorization: "Bearer " + userToken,
      ...
      if (response.message && response.message == "Success") {
        setSuccessAdd(i18n.language === "ar" ? "تم تعديل خيار تسعير بنجاح" : "Pricing Option has been Updated successfully")
        setRefresh(!refresh)
          ...
          <CCardHeader>
            <CRow className=" row-gap-15">
              ...
              <CCol md="6" lg="6" xl="6" className="justify-content-center align-self-center align-items-center place-items-center text-capitalize">
                <strong>{i18n.language === "ar" ? "تعديل خيار تسعير" : "Update Pricing Option"} </strong>
              </CCol>
...
export default PricingOptionUpdate
