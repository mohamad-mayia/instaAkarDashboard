...
import './PropertySiteUpdate.scss'
...
const PropertySiteUpdate = ({ match }) => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const responsee = await fetch(
        `${global.apiUrl}api/propertySites/${match.params.id}?_method=put`,
        {
          method: "POST",
          headers: {
            Authorization: "Bearer " + userToken,
      ...
      if (response.message && response.message == "Success") {
        setSuccessAdd(i18n.language === "ar" ? "تم تعديل ميزة بنجاح" : "Amenity has been Updated successfully")
        setRefresh(!refresh)
          ...
          <CCardHeader>
            <CRow className=" row-gap-15">
              ...
              <CCol md="6" lg="6" xl="6" className="justify-content-center align-self-center align-items-center place-items-center text-capitalize">
                <strong>{i18n.language === "ar" ? "تعديل ميزة" : "Update Amenity"} </strong>
              </CCol>
...
export default PropertySiteUpdate
